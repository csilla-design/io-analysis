...
Thailand's 5 industries of the future, from the 180-sector IO 1990 table.

Mirrors compute_multipliers.py (2021) exactly; only the data loader differs
because the 1990 workbook has a different column layout and does not embed
sector names or the industry-of-the-future mapping. Those are reused from
IO2021 180 sectors.xlsx (the 180-sector classification is identical across
the two years).

1990 file layout (sheet "1990", data from row 4):
  A: Row code
  B: Column code
  C: Old purchaser price
  D: New purchaser price   (we use this — it is the corrected series)
  E: Wholesale trade margin
  F: Retail trade margin
  G: Transport cost
  H: Import

Method and indicators are identical to the 2021 script; see that file's
...
SRC = "IO1990 180 sectors.xlsx"
SRC_MAPPING = "IO2021 180 sectors.xlsx"   # sector names and industry-of-the-future mapping
OUT = "industries_of_the_future_multipliers_1990.xlsx"
...
data = wb["1990"]
# Reuse sector names and industry mapping from the 2021 workbook — the 180-sector
# classification is the same in both years.
wb_map = openpyxl.load_workbook(SRC_MAPPING, data_only=True)
mapping = wb_map["sector mapping"]
...
# Pull sector names from the 2021 workbook (mapping sheet columns: code, IO sector, industry)
for row in mapping.iter_rows(min_row=2, values_only=True):
    code, name, _ = row[0], row[1], row[2]
    if code in code_to_idx:
        sector_name[code] = name

# Iterate 1990 long-format table. Columns are (ROW, COL, OldPurch, NewPurch, Wholesale, Retail, Transport, Import).
for row in data.iter_rows(min_row=4, values_only=True):
    rc, cc, _old_purch, new_purch, whol, retail, trans, imp = row
    if rc is None:
        break
    purch = new_purch or 0
    whol = whol or 0
    retail = retail or 0
    trans = trans or 0
    imp = imp or 0
...
methodology_lines = [
    ...
    ("F. Export-orientation convention. Export orientation is reported as exports (cols 305 + 306) divided by "
     ...
     "domestic trade margins: the exports/output ratio there exceeds 100%, sometimes by orders of magnitude, "
     "because the numerator includes the gross re-exported value while the denominator only captures the small "
     "domestic processing margin. Switching to exports/total-supply puts the ratio on a bounded supply basis "
     "(supply identity: total supply = total demand, with exports as one component of demand) and makes export "
     "and import indicators directly comparable. A handful of sectors still slightly exceed 100% due to negative "
     "inventory changes or trade-margin allocations in the original table; these are reported as-is rather than "
     "capped.",
     False),
]
row = 2
